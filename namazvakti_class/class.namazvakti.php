...
Class Namaz
{
	...
	public function vakit( $ulke=NULL, $sehir=NULL, $ilce=NULL, $cikti='array' )
	{
		...
		{
			$sonuc = $this->__cache_oku( $cache_dosya_adi );
		} else {
	...
	private function al_vakitler( $ulke=NULL, $sehir=NULL, $ilce = NULL )
	{
		...
		if ( $sonuc['durum'] == 'basarili' )
		{
			...
			$location = array(
				'ulke'	=> $ulkeler['veri'][$ulke],
				'sehir'	=> $sehirler['veri'][$sehir],
				'ilce'	=> $sehir == $ilce ? $sehirler['veri'][$sehir] : $ilceler['veri'][$ilce]
			);
			...
			foreach ( $sonuc['veri'] as $k=>$v )
			{
				if( !in_array($k, $karaliste ) )
				{
					...
					elseif ( $k == 'MiladiTarih' )
					{
						$veri[strtolower($k)] = date('d.m.Y');
						$veri['miladitarih2'] = date('d') . ' ' . $this->miladiaylar[date('n')] . ' ' . date('Y');
					} else {
						$veri[strtolower($k)] = $v;
					}
				}
	...
	private function hicri( $tarih = null )
	{
		...
		return $bugun['day'] . ' ' . $this->hicriaylar[$bugun['month']] . ' ' . $bugun['year'];
	}
